...
def filter_for_now(data_t):
    # filter points with no summaries
    uris_wo_summaries = [x['uri'] for x in data_t if not x['labelled_summaries']]
    # merge clarification and treatment into information
    valid_keys = ('INFORMATION', 'CAUSE', 'EXPERIENCE', 'QUESTION', 'SUGGESTION')
    uris_with_other_summaries = [x['uri'] for x in data_t if not set(x['labelled_answer_spans']).issubset(valid_keys)]
    filter = set(uris_wo_summaries + uris_with_other_summaries)
    dataset_filtered = [x for x in data_t if x['uri'] not in filter]
    for e in dataset_filtered:
        raw_text = e['raw_text']
        group_index = raw_text.find("_GROUP")
        raw_text_1 = raw_text[:group_index]
        raw_text_2 = raw_text_1[:raw_text_1.rfind('\n')]
        e['raw_text'] = raw_text_2
    print(f' len of filtered {len(dataset_filtered)}')
    for dp in dataset_filtered:
        add_span_indices(dp)
    #         break
    print(f' len of filtered after adding indices {len(dataset_filtered)}')
    data_tf_fc = final_check(dataset_filtered)
    print(f' len of filtered after adding indices {len(data_tf_fc)}')
    return data_tf_fc


def convert_to_squad_schema(datatf):
    prompt_template_i = "Which text segment provides information relevant to the question?"
    prompt_template_c = "What text segment provides causes for the medical inquiry?"
    prompt_template_s = "Which text segment provides suggestions relevant to the question?"
    prompt_template_e = "Which text segment provides experiences relevant to the question?"
    prompt_template_q = "Which text segment contains a follow up question to the previous question?"
    prompts = {}
    prompts['INFORMATION'] = prompt_template_i
    prompts['SUGGESTION'] = prompt_template_s
    prompts['EXPERIENCE'] = prompt_template_e
    prompts['QUESTION'] = prompt_template_q
    prompts['CAUSE'] = prompt_template_c
    data = []
    data_obj = {}
    data_obj['title'] = "yahoo-l6"
    data_obj['paragraphs'] = []
    for dpt in datatf:
        idx_prefix = int(dpt['uri']) * 10000
        # print(idx_prefix)
        for persp_spans in dpt['labelled_answer_spans']:
            for label_entry in dpt['labelled_answer_spans'][persp_spans]:
                obj = {}
                obj['context'] = label_entry['original_answer']
                qas = {}
                qas['question'] = f"{dpt['question']}.{prompts[persp_spans]}"
                idx_prefix += 1
                qas['id'] = idx_prefix + 1
                qas['is_impossibe'] = False
                qas['answers'] = [{"text": label_entry['mod_txt'], "answer_start": label_entry['answer_index_start']}]
                obj['qas'] = [qas]
                data_obj['paragraphs'].append(obj)
...
def main(args):
    ...
    #dataset_path = "dataset_1700929816.json"
    dataset_path = "/workspace/dataset/SemEval-2024_Task3/text/Subtask_1_train.json"
    import json
    ...
    if args.do_train or (not args.eval_test):
        ...
        eval_examples = read_squad_examples(input_file=eval_dataset, is_training=False, version_2_with_negative = False)
    ...
    if args.do_train:
        ...
        train_examples = read_squad_examples(
            input_file=squad_ds, is_training=True, version_2_with_negative = False)
...
# class Args:
#     def __init__(self, model='spanbert-base-cased', output_dir='./finetuned_squad', train_file='', dev_file='',
#                  test_file=None, eval_per_epoch=10, max_seq_length=512, doc_stride=128, max_query_length=64,
#                  do_train=False, do_eval=True, do_lower_case=False, eval_test=False, train_batch_size=32,
#                  eval_batch_size=32, learning_rate=2e-05, num_train_epochs=4.0, eval_metric='best_f1',
#                  train_mode='random_sorted', warmup_proportion=0.1, n_best_size=20, max_answer_length=30,
#                  verbose_logging=False, no_cuda=False, seed=42, gradient_accumulation_steps=1, fp16=False, loss_scale=0,
...
# Example usage:
# args_instance = Args(model='spanbert-base-cased', output_dir='../finetuned_squad', train_file='train-v2.0.json', dev_file='dev-v2.0.json', test_file=None, eval_per_epoch=10, max_seq_length=512, doc_stride=128, max_query_length=64, do_train=False, do_eval=True, do_lower_case=False, eval_test=False, train_batch_size=32, eval_batch_size=32, learning_rate=2e-05, num_train_epochs=4.0, eval_metric='best_f1', train_mode='random_sorted', warmup_proportion=0.1, n_best_size=20, max_answer_length=30, verbose_logging=False, no_cuda=False, seed=42, gradient_accumulation_steps=1, fp16=False, loss_scale=0, version_2_with_negative=True)
# print(args_instance.model)  # Accessing the 'model' attribute
# args = Args(model='SpanBERT/spanbert-base-cased', output_dir='./finetuned_squad', train_file='', dev_file='',
#             test_file=None, eval_per_epoch=10, max_seq_length=512, doc_stride=128, max_query_length=64, do_train=False,
#             do_eval=True, do_lower_case=False, eval_test=False, train_batch_size=32, eval_batch_size=32,
#             learning_rate=2e-05, num_train_epochs=4.0, eval_metric='best_f1', train_mode='random_sorted',
#             warmup_proportion=0.1, n_best_size=20, max_answer_length=30, verbose_logging=False, no_cuda=False, seed=42,
#             gradient_accumulation_steps=1, fp16=False, loss_scale=0, version_2_with_negative=False)
# print(args.model)
#
#
#
...
#                                                   BertTokenizer,
#                                                   whitespace_tokenize)
# args.model = "spanbert-base-cased"
# tokenizer = BertTokenizer.from_pretrained(args.model, do_lower_case=args.do_lower_case)
#
# train_examples = read_squad_examples(input_file=squad_ds['data'], is_training=True, version_2_with_negative=args.version_2_with_negative)
# # train_features = convert_examples_to_features( examples=train_examples, tokenizer=tokenizer, max_seq_length=512,doc_stride=128,max_query_length=64,is_training=True)
# train_features =convert_examples_to_features(
#                 examples=train_examples,
#                 tokenizer=tokenizer,
#                 max_seq_length=args.max_seq_length,
...
#                 max_query_length=args.max_query_length,
#                 is_training=True)
# print("success training stuff")
#
# eval_examples = read_squad_examples(input_file=squad_ds_eval['data'], is_training=False, version_2_with_negative=args.version_2_with_negative)
# #eval_features = convert_examples_to_features(examples=eval_examples, tokenizer=tokenizer,max_seq_length=512,  doc_stride=128,max_query_length=64,is_training=False)
#
# eval_features =convert_examples_to_features(
#                 examples=eval_examples,
#                 tokenizer=tokenizer,
...
#
# print(f'{all_eval_input_ids.shape} , {all_eval_input_mask.shape}, {all_eval_segment_ids.shape}, {all_eval_example_index.shape} ')
#
# eval_data = TensorDataset(all_eval_input_ids, all_eval_input_mask, all_eval_segment_ids, all_eval_example_index)
# eval_dataloader = DataLoader(eval_data, batch_size=args.eval_batch_size)
#
# if args.train_mode == 'sorted' or args.train_mode == 'random_sorted':
#     train_features = sorted(train_features, key=lambda f: np.sum(f.input_mask))
...
# lrs = [args.learning_rate] if args.learning_rate else \
#     [1e-6, 2e-6, 3e-6, 5e-6, 1e-5, 2e-5, 3e-5, 5e-5]
# print(f"features and batches loaded, len of batches {len(train_batches)}")
#
#
#
#
